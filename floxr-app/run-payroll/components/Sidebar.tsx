...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-64 flex-shrink-0 flex flex-col justify-between bg-white dark:bg-[#111318] border-r border-slate-200 dark:border-[#282e39] transition-colors duration-300 z-20">
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {navItems.map((item) => (
            ...
            >
              <span className={`material-symbols-outlined text-[20px] ${!item.active ? 'group-hover:text-primary' : ''} transition-colors`}>
                {item.icon}
              </span>
